...
class YaUploader:
    ...
    def create_folder(self, path):
        ...
        if response.status_code == 201:
            print(f'Создана папака "{"VK_Id" + vk_id}" на Яндекс.Диске')
        else:
...
def make_local_folder():
    if os.path.exists("VK_Id" + vk_id):
        pass
    else:
        os.mkdir("VK_Id" + vk_id)
...
def download_vk_photos():
    ...
    params = {'owner_id': vk_id, 'album_id': 'profile', 'extended': '1', 'photo_sizes': '1', 'count': photos_count,
    ...
    if 'error' in response.json():
        ...
        for i in response.json().values():
            ...
            for j in i['items']:
                ...
                results = {}
                if os.path.isfile(f'{"VK_Id" + vk_id}/{filename}') is False:
                    filename = f"{j['likes']['count']}.jpg"
                    response = requests.get(url)
                    # results ={}
                    if response.status_code == 200:
                        with open(f'{"VK_Id" + vk_id}/{filename}', 'wb') as file:
                            file.write(response.content)
                            ya.upload(f'{"VK_Id" + vk_id}/{filename}', f'{"VK_Id" + vk_id}/{filename}')
...
def upload_result_json():
    with open(f'{"VK_Id" + vk_id}/results_json.txt', 'w+') as file:
